-- stg_pbi_admin__scanner_dataset_users.sql
-- Source: dlt child table scanner_workspaces__datasets__users
-- One row per (dataset, user) access entry.
-- Join to stg_pbi_admin__scanner_datasets on _dlt_parent_id = _dlt_id.

{% set relation = adapter.get_relation(
    database   = 'platform',
    schema     = 'raw_pbi_admin',
    identifier = 'scanner_workspaces__datasets__users'
) %}
...
select
    ...
    graph_id,
    principal_type,
    dataset_user_access_right       as access_right,
    user_type,

    _dlt_load_id,
    _dlt_id

from {{ source('raw_pbi_admin', 'scanner_workspaces__datasets__users') }}
